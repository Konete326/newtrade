...
export default function CustomerDetail() {
  ...
  useEffect(() => {
    Promise.all([customerService.getById(id), customerService.getLedger(id)])
      .then(([cRes, lRes]) => { setCustomer(cRes.data.data); setLedger(lRes.data.data || []); })
  ...
  return (
    <div className="p-4 lg:p-6">
      <PageHeader title={customer.name} subtitle={`${customer.type || 'CUSTOMER'} — ${customer.phone || 'No phone'}`} backPath="/customers" />
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900"><p className="text-xs text-gray-500">Email</p><p className="mt-1 text-sm font-medium text-gray-800 dark:text-gray-200">{customer.email || '-'}</p></div>
          <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900"><p className="text-xs text-gray-500">Phone</p><p className="mt-1 text-sm font-medium text-gray-800 dark:text-gray-200">{customer.phone || '-'}</p></div>
          <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900"><p className="text-xs text-gray-500">Address</p><p className="mt-1 text-sm font-medium text-gray-800 dark:text-gray-200">{customer.address?.line1 || customer.address || '-'}</p></div>
          <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900"><p className="text-xs text-gray-500">Outstanding</p><p className="mt-1 text-sm font-bold text-error-600">{formatCurrency(customer.outstanding || 0)}</p></div>
        </div>
